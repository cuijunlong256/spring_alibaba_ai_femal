...
/**
 * 针对男性用户提供女性红娘工作流的风险评估智能体
 * 检测自杀/自伤倾向、涉黄、涉政、涉暴、涉恐、涉毒等风险内容
 */
@Configuration
public class FemaleMatchmakerRiskAgentConfig {
    ...
    @Bean
    @Qualifier("femaleMatchmakerRiskAgent")
    public ReactAgent femaleMatchmakerRiskAgent(
            @Qualifier("flashChatModel") ChatModel chatModel) {
        return ReactAgent.builder()
                .name("femaleMatchmakerRiskAgent")
                .model(chatModel)
                .instruction(INSTRUCTION)
                .description("情感红娘安全审核：检测自杀/自伤倾向及涉黄涉政等违规内容")
                .outputKey("femaleMatchmakerRiskResult")
